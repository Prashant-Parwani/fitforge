...
export default function Progress() {
  ...
  // All data stored per-user in their profile
  ...
  const [logForm, setLogForm] = useState({
    ...
    sets: '', duration: '', notes: '', date: new Date().toISOString().split('T')[0]
  })
  ...
  const saveWeight = () => {
    ...
    const entry = { label, value: val, date: new Date().toISOString().split('T')[0] }
    updateUser({ weightLog: [...weightLog, entry] })
    setNewWeight('')
  }

  const deleteWeight = (idx) => updateUser({ weightLog: weightLog.filter((_, i) => i !== idx) })

  // Stats derived from real log
      ...
      <section className="bg-brown-800 text-cream py-12 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-brown-300 font-body max-w-2xl">
            Every rep, every kg — tracked. Your data starts fresh from day one and grows with you.
          </p>
          {user?.startDate && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex gap-1 py-2">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab === t.id ? 'bg-brown-500 text-cream' : 'text-brown-600 hover:bg-brown-100'}`}>
              {t.emoji} {t.label}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-6">
            ...
            {totalWorkouts === 0 && weightLog.length === 0 ? (
              <div className="text-center py-20 bg-cream rounded-2xl border border-brown-200">
                ...
                <p className="text-brown-500 font-body mb-6 max-w-sm mx-auto">
                  No workouts logged yet. Log your first session or enter your starting weight to begin tracking.
                </p>
              ...
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    { label:'Weight Change', value: weightChange !== null ? (weightChange >= 0 ? '+'+weightChange : weightChange)+' kg' : '—',
                      unit: weightChange !== null ? (weightChange <= 0 ? 'kg lost' : 'kg gained') : 'no data yet',
                    ...
                    <div className="divide-y divide-brown-100">
                      {workoutLog.slice(0,8).map(w => (
                        <div key={w.id} className="flex items-center gap-4 px-5 py-3 hover:bg-brown-50 group">
                          ...
                          <button onClick={() => deleteWorkout(w.id)} className="text-brown-200 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-sm ml-1">🗑</button>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="bg-cream rounded-2xl border border-brown-200 p-5 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {MUSCLES.map(m => (
                    <button key={m} type="button" onClick={() => setLogForm({...logForm, focus: m})}
                      className={`py-1.5 px-3 rounded-full text-xs font-medium border transition-all ${logForm.focus === m ? 'bg-brown-500 text-cream border-brown-500' : 'border-brown-200 text-brown-600 hover:border-brown-400'}`}>
                      {m}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" placeholder="e.g. 18" value={logForm.sets} onChange={e => setLogForm({...logForm, sets: e.target.value})}
                    className="w-full px-3 py-2.5 rounded-xl border border-brown-200 bg-brown-50 text-brown-800 text-sm focus:outline-none focus:border-brown-400" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea placeholder="e.g. New PB on bench! Felt strong today." value={logForm.notes}
                  onChange={e => setLogForm({...logForm, notes: e.target.value})}
                  rows={3} className="w-full px-3 py-2.5 rounded-xl border border-brown-200 bg-brown-50 text-brown-800 text-sm focus:outline-none focus:border-brown-400 resize-none" />
              </div>
                ...
                <div className="divide-y divide-brown-100 max-h-72 overflow-y-auto">
                  {workoutLog.map(w => (
                    <div key={w.id} className="flex items-center gap-3 px-5 py-3 group hover:bg-brown-50">
                      <div className="w-9 h-9 bg-brown-500 text-cream rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0">{(w.focus||'?')[0]}</div>
                      <div className="flex-1 min-w-0">
                      ...
                      <button onClick={() => deleteWorkout(w.id)} className="text-brown-200 hover:text-red-400 opacity-0 group-hover:opacity-100 text-sm">🗑</button>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {tab === 'weight' && (
          <div className="max-w-lg mx-auto space-y-5">
            ...
            <div className="bg-cream rounded-2xl border border-brown-200 p-5">
              ...
              <p className="text-xs text-brown-400 mt-2">💡 Weigh yourself first thing in the morning, before eating, for accuracy.</p>
            </div>
                ...
                <div className="bg-cream rounded-2xl border border-brown-200 overflow-hidden">
                  ...
                  <div className="divide-y divide-brown-100 max-h-64 overflow-y-auto">
                    {[...weightLog].reverse().map((w, i) => {
                      const originalIdx = weightLog.length - 1 - i
                      const prev = weightLog[originalIdx - 1]
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between px-5 py-3 group hover:bg-brown-50">
                          ...
                          <button onClick={() => deleteWeight(weightLog.length - 1 - i)} className="text-brown-200 hover:text-red-400 opacity-0 group-hover:opacity-100 text-sm">🗑</button>
                        </div>
                      )
                    })}
                  </div>
            ...
            {user?.targetWeight && currentWeight && (
              <div className="bg-cream rounded-2xl border border-brown-200 p-5">
                ...
                {(() => {
                  ...
                  const pct          = totalChange > 0 ? Math.min(100, Math.round((achieved / totalChange) * 100)) : 0
                  ...
                  return (
                    <div>
                      ...
                      <div className="h-3 bg-brown-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-700 ${losing ? 'bg-green-500' : 'bg-blue-500'}`}
                          style={{ width: `${pct}%` }} />
                      </div>
                      <p className="text-xs text-brown-500 mt-2">
                        {pct >= 100
                          ? '🎉 Target reached! Set a new goal.'
                          : losing
                            ? `${(current - target).toFixed(1)} kg to go`
                            : `${(target - current).toFixed(1)} kg to go`}
                      </p>
                    </div>
                  )
                })()}
            ...
            <div className="bg-brown-800 text-cream rounded-2xl p-5">
              ...
              <ul className="text-brown-300 text-sm space-y-1 list-disc pl-4 font-body">
                ...
                <li>Bulking: aim for 0.5–1 kg/week with progressive overload</li>
              </ul>
            </div>
          </div>
        )}
